...
- Schema migrations are forward-only, numbered (`v001`, `v002`, ...), tracked in `system_config` table. Current head is `v007_reminder.surql`, and `schema::LATEST_VERSION` is the single source for that number (migration tests assert against it, never a literal).
