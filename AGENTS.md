...
- The HNSW index on `fact.embedding` is defined at boot by `schema::ensure_vector_index()`, not in a
  ...
  exists. `<|k,ef|>` without an index strips to a plain scan, so `do_retrieve_memories` picks by
