...
- **storage** owns all DB access — no raw SurrealDB queries outside this crate. Two exceptions survive, both maintained elsewhere; don't add a third: the `alexandria-mcp` `provenance` create in `do_store_memory`, and the binary's cluster-maintenance loop in `main.rs`, which reads `cluster` with raw SQL twice per tick
...
- **mcp** wires tools to engine+storage. It is not the only crate that sees both — the binary depends on engine *and* storage, and `main.rs:serve_http`'s maintenance task combines them — but it is the only one whose *purpose* is joining them. Also owns the debug web UI (`alexandria-mcp/src/debug/`), which is Axum handlers over the same repos, plus `crates/alexandria-mcp/templates/` (askama, compiled at build time) and `crates/alexandria-mcp/assets/` (vendored third-party JS, `include_bytes!`). Both are **compile-time contracts**: a missing template or asset file is a build error, not a runtime 404.
...
- `status` has **three** values, not the two the original design specified: `pending | delivered | cancelled`. `'delivered'` exists because a consumed one-shot gets a NULL `next_due_at`, and `NULL <= $now` still matches in SurrealQL — so NULL cannot mark "never fires again" and an explicit terminal value is the only honest option. It is asserted in the v007 schema and filtered on by `list_reminders`.
- `set_reminder` returns `next_fire_preview` whose **first element *is* `next_due_at`**, not a separate list alongside it. The design asked for "next_due_at plus the next 3"; this is the same four instants minus the duplicate, and it puts the value the caller confirms against first.
- `check_reminders` takes a **flat `project`** param, not the design's `context: { project }` wrapper — it matches the other reminder tools and avoids a one-key object.
- Project escalation is **inclusive**: a project-targeted reminder escalates at `>= escalation_hours` overdue. That is what makes `escalation_hours = 0` mean "escalate as soon as overdue"; a strict `>` would make 0 mean never.
- `set_reminder` rejects a blank `message` and v007 asserts non-empty after trim. The client-side `(reminder with no text)` placeholder is defence in depth, not the enforcement point.
- `normalize_cron` collapses whitespace on **every** branch, so a padded expression cannot render two different `schedule` strings from one row.
- **Claude Code has no automatic reminder delivery.** `contrib/claude/hooks/alexandria-recall.sh` injects auto-recall only; a Claude Code user sees the read-only `due_reminders` piggyback but no `check_reminders` consumption unless the agent calls the tool itself. The pi companion extension is the shipped delivery path — implementing it in the hook was deliberately left out of scope.
- The pi extension's three features (recall, store, reminders) are **config-guarded modules dispatched from one handler**, not implementations of a `Feature` interface — a formal interface would be premature abstraction for three features. The dispatcher still gives per-feature failure isolation. Their blocks are merged into a **single** `before_agent_start` injection (customType `alexandria`) for deterministic ordering and exactly one injected message per prompt; pi would merge several handlers' messages anyway, so this is a choice, not a workaround.
- The companion's `project` hint comes from the session's `ctx.cwd`, **cached per directory** rather than once per process, and the recall/reminder merge lives in `src/injection.ts` so the isolation claim is tested rather than inspected. The whole prompt path carries one `PROMPT_BUDGET_MS` deadline, and `resetClient()` closes what it drops.
...
- Every definition statement in a migration is `DEFINE <kind> OVERWRITE …` and every removal is `REMOVE <kind> IF EXISTS …` — with one survivor: `v003` opens `DEFINE TABLE IF NOT EXISTS system_config SCHEMAFULL`, which is replay-safe and so does not break the gate. **The rule exists because `migrate()` is not atomic across a file**: each migration is one multi-statement query, SurrealDB commits each statement in its own transaction, and `system_config` is stamped only after all pending migrations succeed. A crash mid-file leaves definitions applied with the old stamp, so the next boot re-runs that file from the top — with plain `DEFINE` that is a deterministic `The table 'reminder' already exists` and the server never starts again. `migration_test::test_replaying_a_completed_migration_is_not_an_error` re-applies every compiled-in file and is the gate on the rule; `MIGRATIONS` is `pub` for exactly that test. Note the 3.2 grammar puts the qualifier *after* the kind — `DEFINE OVERWRITE FIELD …` is a parse error.
- Embedding model is locked on first boot — changing `config.toml` model without wiping data will refuse to start. The truncation limit (`embedding.max_tokens`, default `DEFAULT_MAX_TOKENS` = 128 in `candle.rs`, 256 tested, up to 512) is locked the same way as `embedding_max_tokens`. A lock without that key, or facts with no lock at all, means the corpus was embedded at the tokenizer's shipped 128 (`PRE_LOCK_MAX_TOKENS`), and that is what gets compared or stamped, never the configured value. The limit only goes up: boot refuses a mismatch in either direction, and `alexandria migrate-embeddings` re-embeds upward and refuses downward. The lock check takes `CandleProvider::max_tokens()`, not the config value, so it records what the vectors were made with.
...
- Integration tests use `Database::connect_embedded()` (in-memory SurrealDB), so no disk state leaks between them. One deliberate exception opens a persistent SurrealKV store under a `tempfile::tempdir()` and reconnects to it, to prove round-trip survival; the tempdir keeps it isolated all the same.
...
- `AGENTS.md` — this file. It was named `CLAUDE.md` until the docs sweep that added session memory
  and the pi extension docs, so references to `CLAUDE.md` in commits before that point mean this file.
