...
- Run tests on **stable**, not nightly: `diskann-wide` (SurrealDB transitive dep) fails trait inference on its NEON intrinsics under recent nightlies on aarch64, and the failure looks like it originates in this workspace. Current suite: 344 tests, all green — recount with `cargo test --all-features 2>&1 | rg -o '([0-9]+) passed' | awk '{s+=$1} END {print s}'` rather than trusting the number here.
